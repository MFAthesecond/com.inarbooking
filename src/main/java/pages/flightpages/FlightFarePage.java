...
public class FlightFarePage extends BasePage {
	...
	public List<String> getContainerInfo(String containerName) {
		List<WebElement> containerInfo = getContainer(containerName)
			.findElements(By.cssSelector("div[class='col-12 fs-3 text-muted'] span"));
	...
	public String getContainerDirection(String containerName) {
		return getContainer(containerName)
			.findElement(By.cssSelector("div[class='col-12 display-2 fw-semibold text-dark']"))
			.getText();
	}
	...
	private WebElement getFareContainer(String containerName, String fareType) {
		...
		WebElement fare = fareTypes.stream()
			.filter(element -> element.findElement(By.cssSelector(".fare-item-inner .fare-item-header span"))
				.getText()
				.equals(fareType))
			.findFirst()
			.get();
	...
	public void clickOnFareType(String containerName, String fareType) {
		BrowserUtils.clickOnElement(getFareContainer(containerName, fareType).findElement(By.cssSelector(".icn")));
	}

	public List<String> getFareIncludeInfo(String containerName, String fareType) {
		List<WebElement> fareInfoList = getFareContainer(containerName, fareType)
			.findElements(By.cssSelector(".fare-item-body > div:nth-child(1) > div > span:nth-child(2)"));
		return fareInfoList.stream().map(WebElement::getText).toList();
	}

	public String getFarePrice(String containerName, String fareType) {
		return getFareContainer(containerName, fareType)
			.findElement(By.cssSelector(".fare-item-inner .mt-5 span:nth-child(2)"))
			.getText();
	}
	...
	public void clickOnBackButton() {
		BrowserUtils.clickOnElement(returnContainer.findElement(By.cssSelector("button:nth-child(1)")));
	}

	public void clickOnSelectWhoFlyingButton() {
		returnContainer.findElement(By.cssSelector("button:nth-child(2)"));

		BrowserUtils.clickOnElement(returnContainer.findElement(By.cssSelector("button:nth-child(2)")));
	}
	...
	public boolean isFareTypeSelected(String containerName, String fareType) {
		WebElement fare = getContainer(containerName).findElements(By.cssSelector(".fare-item-header"))
			.stream()
			.filter(element -> element.findElement(By.cssSelector("span")).getText().equals(fareType))
			.findFirst()
			.get();
