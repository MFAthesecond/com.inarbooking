...
public class HotelSelectionPage extends BasePage {
	...
	public void selectThingsToDo(List<String> selectionThingList) {
		...
		List<String> textList = funThingsToDoNameList.stream().map(WebElement::getText).toList();
	...
	public void selectAdditionalFeatures(List<String> selectionAdditionalList) {
		scrollToElement(maxPriceSlider);
		List<String> textList = additionalFeaturesNameList.stream().map(WebElement::getText).toList();
	...
	public void selectHotelTypes(List<String> stringList) {
		...
		for (int i = 0; i < stringList.size(); i++) {
			final int a = i;
			selectedTypesList = listOfHotelTypesStrings.stream()
				.filter(e -> e.getText().contains(stringList.get(a)))
				.toList();
		}
