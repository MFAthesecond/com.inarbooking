...
public class HotelBookingSteps extends BaseStep {
	...
	public void validate_the_user_is_on_hotel_selection_page() {
		LOGGER.debug("Validating that the user is on the hotel selection page");
		try {
			boolean isOnHotelSelectionPage = PAGES.getHotelPages()
				.getHotelSelectionPage()
				.validateNavigationToHotelSelectionPage();
			LOGGER.debug("Validation result: {}", isOnHotelSelectionPage);
			assertThat(isOnHotelSelectionPage).as("Verify that the user is on the hotel selection page").isTrue();
		}
		catch (Exception e) {
			LOGGER.error("An error occurred while validating hotel selection page: {}", e.getMessage());
			throw new AssertionError("Error occurred while validating hotel selection page.", e);
		}
	}
