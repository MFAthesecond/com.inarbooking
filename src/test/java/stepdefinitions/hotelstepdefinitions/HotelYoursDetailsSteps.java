...
public class HotelYoursDetailsSteps extends BaseStep {
	...
	public void verify_that_you_can_pass_the_final_step_page() {
		LOGGER.debug("Verifying that the user can pass the final step page");
		try {
			boolean isPassed = PAGES.getHotelPages().getHotelsPaymentPage().validateNavigationToFinalPage();
			LOGGER.info("Navigation result: {}", isPassed);
			assertThat(isPassed).as("Verify that the final step page is passed successfully").isTrue();
		}
		catch (Exception e) {
			LOGGER.error("An error occurred while verifying final step page: {}", e.getMessage());
			throw new AssertionError("Error occurred while verifying final step page.", e);
		}
	}
