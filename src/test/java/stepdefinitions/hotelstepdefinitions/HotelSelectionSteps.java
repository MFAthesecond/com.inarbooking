...
public class HotelSelectionSteps extends BaseStep {
	...
	public void verify_that_the_user_is_on_the_details_page() {
		...
		try {
			...
			assertThat(isOnDetailsPage).as("Verify that the user is on the details page").isTrue();
		}
		catch (Exception e) {
	...
	public void verifyThatAndForFunThingsToDoAreSelected(String arg0, String arg1) {
		LOGGER.debug("Verifying that {} and {} for fun things to do are selected", arg0, arg1);
		try {
			boolean areSelected = PAGES.getHotelPages()
				.getHotelDetailsPage()
				.validateHotelAspects(Arrays.asList(arg0, arg1));
			LOGGER.debug("Verification result: {}", areSelected);
			assertThat(areSelected).as("Verify that {} and {} for fun things to do are selected", arg0, arg1).isTrue();
		}
		catch (Exception e) {
			LOGGER.error("An error occurred while verifying fun things to do: {}", e.getMessage());
			throw new AssertionError("Error occurred while verifying fun things to do.", e);
		}
	}
