...
public class HotelPaymentSteps extends BaseStep {
	...
	public void write_the_card_holder_s_name_as(String cardName) {
		PAGES.getHotelPages().getHotelsFinalStep().fillCardHolderName(cardName);
	...
	public void enter_card_number_as(String cardNumber) {
		PAGES.getHotelPages().getHotelsFinalStep().fillCardNumber(cardNumber);
	...
	public void enter_expiration_date_as(String date) {
		PAGES.getHotelPages().getHotelsFinalStep().fillExpirationDate(date);
		LOGGER.debug("Filled expiration date: {}", date);
	}

	@When("Enter cvv as {string}")
	public void enter_cvv_as(String ccv) {
		PAGES.getHotelPages().getHotelsFinalStep().fillCvcCode(ccv);
	...
	public void verify_that_the_user_is_on_the_payment_page() {
		assertThat(PAGES.getHotelPages().getHotelsFinalStep().validateInfoTitle()).isTrue();
		try {
			LOGGER.info("Verifying that the user is on the payment page...");
			boolean isInfoTitleDisplayed = PAGES.getHotelPages().getHotelsFinalStep().validateInfoTitle();
			assertThat(PAGES.getHotelPages().getHotelsFinalStep().validateInfoTitle()).isTrue();
	...
	public void verifyThatSelected(String arg0) {
		Assertions.assertThat(arg0.toString()).isEqualTo(PAGES.getHotelPages().getHotelPage().getHotelsField());
