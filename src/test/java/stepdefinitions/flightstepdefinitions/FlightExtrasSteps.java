...
public class FlightExtrasSteps extends BaseStep {
	...
	public void verifyThatIsSelected(String mealName, String extra1Name, String extra2Name) {
		try {
			LOGGER.debug("Verifying selections - meal: {}, extra 1: {}, extra 2: {}.", mealName, extra1Name,
					extra2Name);
	...
	public void verifyThatTotalPriceCalculationIsCorrectAfterSelections(String mealName, String extra1Name,
			String extra2Name) {
		try {
			LOGGER.debug("Verifying total price calculation after selections - meal: {}, extra 1: {}, extra 2: {}.",
					mealName, extra1Name, extra2Name);
			...
			if (!extra1Name.equals("Travel Insurance") && !extra2Name.equals("Travel Insurance")) {
				expectedTotalPrice = totalPrice + FlightHomeSteps.totalTravelerNumber * (Double.parseDouble(extra1Price)
						+ Double.parseDouble(extra2Price) + Double.parseDouble(mealPrice));
			}
			else if (extra1Name.equals("Travel Insurance")) {
				expectedTotalPrice = totalPrice + Double.parseDouble(extra1Price) + FlightHomeSteps.totalTravelerNumber
						* (Double.parseDouble(extra2Price) + Double.parseDouble(mealPrice));
			}
			else {
				expectedTotalPrice = totalPrice
						+ FlightHomeSteps.totalTravelerNumber
								* (Double.parseDouble(extra1Price) + Double.parseDouble(mealPrice))
						+ Double.parseDouble(extra2Price);
			}
			...
			then(PAGES.getFlightPages().getFlightExtrasPage().getTotalPrice())
				.isEqualTo(String.format("%.2f", expectedTotalPrice));
			LOGGER.debug("Verification successful.");
		}
		catch (Exception e) {
