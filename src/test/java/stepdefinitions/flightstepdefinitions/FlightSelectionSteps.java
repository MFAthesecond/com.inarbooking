...
public class FlightSelectionSteps extends BaseStep {
    ...
    public void clickOnSelectReturnTicketButtonForFlight(int flightIndex) {
        try {
            ...
            LOGGER.error("Error occurred while clicking on select return ticket button for flight #{}.", flightIndex, e);
            throw e;
        }
    }
    ...
    public void verifyThatForOriginForDestinationForAdultForChildrenDropdown(String originName, String destinationName, String adultNum, String childrenNum) {
        try {
            LOGGER.debug("Verifying that {} for origin, {} for destination, {} for adult, {} for children dropdown is selected.", originName, destinationName, adultNum, childrenNum);
            ...
            then(PAGES.getFlightPages().getFlightSelectionPage().isElementOnDropDownSelected(destinationName, "Destination"))
                    .as("Destination dropdown selection check")
                    .isTrue();

            then(PAGES.getFlightPages().getFlightSelectionPage().isElementOnDropDownSelected(adultNum, "AdultForRoundTrip"))
                    .as("Adult dropdown selection check")
                    .isTrue();

            then(PAGES.getFlightPages().getFlightSelectionPage().isElementOnDropDownSelected(childrenNum, "ChildrenForRoundTrip"))
    ...
    public void verifyThatAndUnselectedForForCabinClassSelectedForAirlinesSelectedForDuration(String cabinClassName1, String cabinClassName2, String airlineName, String duration) {
        try {
            LOGGER.debug("Verifying that {} and {} unselected for for cabin class, {} selected for airlines, {} selected for duration.", cabinClassName1, cabinClassName2, airlineName, duration);
            then(PAGES.getFlightPages().getFlightSelectionPage().isElementSelectedOnList(cabinClassName1, "CabinClassForRoundTrip"))
                    .as("Cabin class 1 selection check")
                    .isFalse();

            then(PAGES.getFlightPages().getFlightSelectionPage().isElementSelectedOnList(cabinClassName2, "CabinClassForRoundTrip"))
                    .as("Cabin class 2 selection check")
                    .isFalse();

            then(PAGES.getFlightPages().getFlightSelectionPage().isElementSelectedOnList(airlineName, "AirlinesForRoundTrip"))
                    .as("Airlines selection check")
                    .isTrue();

            then(PAGES.getFlightPages().getFlightSelectionPage().isElementSelectedOnList(duration, "DurationForRoundTrip"))
                    .as("Duration selection check")
                    .isTrue();
            LOGGER.debug("Verification successful.");
        } catch (Exception e) {
    ...
    public void getTheFlightInformationAndClickOnSelectTicketButton(int flightIndex) {
        try {
            ...
            LOGGER.error("Error occurred while getting flight information and clicking on select ticket button for flight #{}.", flightIndex, e);
            throw e;
        }
    }
    ...
    public void getTheFlightInformationAndClickOnSelectReturnTicketButton(int flightIndex) {
        try {
            LOGGER.debug("Getting the #{} flight information and clicking on select return ticket button.", flightIndex);
            ...
            LOGGER.error("Error occurred while getting flight information and clicking on select return ticket button for flight #{}.", flightIndex, e);
            throw e;
        }
    }
}
