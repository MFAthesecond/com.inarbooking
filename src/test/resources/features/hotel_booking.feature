...
Feature: Stay Filtering
  ...
  Scenario: Hotel Reservation with Visitor Details and Finalization
    ...
    Then Verify that the user is on the payment step page
  ...
  Scenario: Complete Hotel Booking with Visitor and Payment Details
    ...
    When Fill in first name "Furkan" last name "Altun" for visitor
  ...
  @Regression
  Scenario: V
  alidate the stays selection process
  ...
  Scenario Outline: Validate that the text area, slide, checkboxes and radio buttons functionality for round trip
    ...
    Examples:
      ...
      | New York    | 3        | 4        | Cycling        | Walking Tours  | Baggage storage     | Daily housekeeping  | Sunset Beach Resort | 3          |
  ...
  Scenario Outline:Verify the functionality of the hotel booking tab with end-to-end testing
    ...
    And Select both checkboxes to receive notification emails
    And Click on the complete booking button
    Then Confirm that you have successfully navigated to the "<Hotel>" confirmation page
    And Click on the close button

    Examples:
      | destination | adultnum | childnum | NumberRoom | FunThingsToDo1 | FunThingsToDo2 | AdditionalFeatures1 | AdditionalFeatures2 | Hotel              | firstName | lastName | contactEmail            | requests             | countryCode | phoneNumber | cardHolderName | cardNumber       | expirationDate | cvv |
      | Amsterdam   | 3        | 4        | 2          | Bicycle rental | Fitness        | Free WiFi           | Elevator            | Coastal Breeze Inn | Furkan    | Altun    | frknmail@gmail.com      | cookie               | +1 (US)     | 7772228882  | Furkan Altun   | 7772228887779994 | 1222           | 777 |
      | Paris       | 2        | 2        | 1          | Walking Tours  | Cycling        | Heating             | Baggage storage     | City Center Suites | Emma      | Johnson  | emma.j@example.com      | No special requests  | +33 (FR)    | 33611223344 | Emma Johnson   | 5555444433331111 | 0523           | 123 |
      | New York    | 4        | 1        | 3          | Skate Parkour  | Fitness        | Private Parking     | Baggage storage     | City Center Suites | James     | Smith    | james.smith@example.com | Extra pillows please | +1 (US)     | 6465551234  | James Smith    | 4111111111111111 | 0624           | 456 |
